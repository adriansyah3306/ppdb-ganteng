...
if (mysqli_query($conn, $siswa_query)) {
    $last_siswa_id = mysqli_insert_id($conn); // Dapatkan ID terakhir siswa yang dimasukkan
    ...
    if (!mysqli_query($conn, $ortu_query)) {
        echo "Error: " . $ortu_query . "<br>" . mysqli_error($conn);
    }
    ...
    if (!mysqli_query($conn, $wali_query)) {
        echo "Error: " . $wali_query . "<br>" . mysqli_error($conn);
    }

    // Jika semuanya berhasil, alihkan ke halaman data siswa
    header("location:../../data_siswa.php");
} else {
    echo "Error: " . $siswa_query . "<br>" . mysqli_error($conn);
}
